...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      data.map((p) => ({ ...p, id: p.id.toString() }))
    );
  } catch (e) {
    console.error("GET /api/penggiat error:", e);
    return NextResponse.json({ message: "Gagal mengambil data penggiat" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.namaInstansi) {
      return NextResponse.json({ message: "Nama instansi wajib" }, { status: 400 });
    }
    ...
    return NextResponse.json({ ...penggiat, id: penggiat.id.toString() }, { status: 201 });
  } catch (e) {
    console.error("POST /api/penggiat error:", e);
    return NextResponse.json({ message: "Gagal menyimpan penggiat" }, { status: 500 });
  }
}
