...
export default function LaporanKerjasamaPage() {
  ...
  const handleExportExcel = async () => {
    ...
    const createSheet = (sheetName: string, data: RepoDoc[]) => {
      ...
      if (data.length === 0) {
        const emptyRow = sheet.addRow({ jenis: "Tidak ada data" });
      ...
      data.forEach((d, i) => {
        ...
        row.eachCell((cell) => {
          cell.border = { top: { style: "thin" }, left: { style: "thin" }, bottom: { style: "thin" }, right: { style: "thin" } };
          if (i % 2 === 1) cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFF1F5F9" } };
        });
    ...
    if (tahun === "all") {
      const years = [...new Set(raw.map((d) => new Date(d.tglMulai).getFullYear()))].sort((a, b) => b - a);
      years.forEach((year) => {
        const yearData = raw.filter((d) => new Date(d.tglMulai).getFullYear() === year);
        createSheet(`Tahun ${year}`, yearData);
      });
      if (years.length === 0) createSheet("Tidak Ada Data", []);
    } else {
      createSheet(`Tahun ${tahun}`, filteredExportData);
    }

    const buffer = await workbook.xlsx.writeBuffer();
    saveAs(new Blob([buffer]), tahun === "all" ? "laporan_kerjasama.xlsx" : `laporan_kerjasama_${tahun}.xlsx`);
  };
