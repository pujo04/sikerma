  ...
  try {
    ...
    const repository = await prisma.repository.create({
      data: {
        ...
        jumlahAnggaran:
          body.jumlahAnggaran !== null &&
          body.jumlahAnggaran !== undefined &&
          body.jumlahAnggaran !== ""
            ? Number(body.jumlahAnggaran)
            : null,

        unitPenanggungJawab:
          body.unitPenanggungJawab?.trim() || "",

        namaPenanggungJawab:
          body.namaPenanggungJawab?.trim() || "",

        // 🔥 SIMPAN PEMBUAT
        createdByUnitId:
          user.role === "UNIT_KERJA" ? user.unitId : null,

        createdBySubUnitId:
          user.role === "SUB_UNIT" ? user.subUnitId : null,
      },
    });
