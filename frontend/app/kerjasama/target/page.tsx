...
export default function TargetKerjasamaPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(API.targetKerjasama);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(API.targetKerjasamaDetail(selectedTarget.id), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(API.targetKerjasamaDetail(deleteTarget.id), { method: "DELETE" });
      if (!res.ok) throw new Error();
